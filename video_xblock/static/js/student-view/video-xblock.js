...
function VideoXBlockStudentViewInit(runtime, element) {
    ...
    function sendBeaconData(handlerUrl, data) {
        fetch(handlerUrl, {
            ...
            if (!response.ok) {
                console.warn('Beacon progress ping failed:', response.status);  // eslint-disable-line no-console
            }
        }).catch(function(reason) {
            console.warn('Beacon progress ping failed:', reason);  // eslint-disable-line no-console
        });
    }
